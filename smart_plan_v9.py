...
def build_workout(sport, intensity, dur, phase, tp, ftp, goal_targets=None, css=None):
    ...
    if sport == "run":
        ...
        if intensity == "hard":
            ...
            #   B: 1km VO2maxインターバル × N
            #   C: ファルトレク (30秒全力/60秒ジョグ × N)
            #   D: 400mスピードレップ × N (最高速度開発)
            _type = _seed_val % 4
            ...
            # 3タイプ: A=テンポ連続, B=クルーズインターバル, C=ビルドアップ
            _type = _seed_val % 3
            wu_min = 10; cd_min = 5; main_min = dur - wu_min - cd_min
            ...
            # 3タイプ: A=Z2ステディ, B=ストライドあり, C=ロング走
            _type = _seed_val % 3
    ...
    if sport == "bike":
        ...
        if intensity == "hard":
            # 5タイプ: A=閾値8分, B=VO2max5分, C=オーバー/アンダー, D=20minSS, E=30秒スプリント
            _type = _seed_val % 5
