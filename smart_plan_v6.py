"""
smart_plan_v6.py — トレーニングプラン生成 v6
=============================================
【v6 追加機能】
...
使い方:
  python smart_plan_v6.py --preview          # 10日プレビュー
  python smart_plan_v6.py --days 14          # 14日分
  python smart_plan_v6.py --today            # 当日のみ
"""
...
def fetch_athlete_data(cfg):
    ...
    # activities_detail.csv の存在を確認して表示
    csv_candidates = [
        Path(__file__).parent / "activities_detail.csv",
        Path(__file__).parent.parent / "activities_detail.csv",
        Path.cwd() / "activities_detail.csv",
    ]
    _csv_found = next((p for p in csv_candidates if p.exists()), None)
    if _csv_found:
        try:
        ...
        print(f"  ℹ️  activities_detail.csv 未配置 (あると過去レーススプリットを自動取得できます)")
...
def _swim_pace_icu(sec_per_100m):
    """秒/100m → intervals.icu workout_doc用ペース表記 (@M:SS)
    intervals.icuのswim workout_docでは "/100m" は不要。
    例: "- 400m @2:23" が正しい構文。
    """
    m, s = divmod(int(sec_per_100m), 60)
    return f"@{m}:{s:02d}"
    ...
    if sport == "run":
        ...
        if intensity == "hard":
            ...
            desc_text = (
                f"【{label}】テンポラン {dur}分\n"
                f"目的: {purpose}\n"
                f"W-up {wu}分 → テンポ {lo}〜{hi} {main}分 → C-down {cd}分\n"
                f"推定距離: {km}km\n"
                f"💪 {motivation}"
            )
...
def consistency_check(plan):
    ...
    for i, d in enumerate(dates):
        ...
        # ── スイム+レスト → レスト削除 ──
        has_swim = any(x["sport"] == "swim" for x in items)
        if has_swim and any(x["sport"] == "rest" for x in items) and len(items) > 1:
            items = [x for x in items if x["sport"] != "rest"]
            notes.append(f"{d}: スイム+レスト → レスト削除")
...
# 出力
# ============================================================
def print_plan(plan, race_info, cond_info, athlete, goal_targets, cfg=None, today_mode=False, str_prog=None):
        ...
        if any(k.lower() in tl for k in RACE_KWS):
            ...
            if found_file:
                ...
                if file_result.get("splits"):
                    ...
                    csv_path = Path(__file__).parent / "activities_detail.csv"
                    if not csv_path.exists():
                        csv_path = Path(__file__).parent.parent / "activities_detail.csv"
                    if csv_path.exists():
                        kws = title.split()[:3]
...
# サマリ3: 今週の仕事スケジュール
# ============================================================
def print_work_schedule_summary(gcal_days, start_date):
    DOW_JP = ["月","火","水","木","金","土","日"]

    print(f"\n{'═'*64}")
    print(f"  🗓  今週の仕事・練習スケジュール  "
          f"({start_date.strftime('%m/%d')}〜{(start_date+timedelta(days=6)).strftime('%m/%d')})")
    print(f"{'═'*64}")

    if not gcal_days:
        print("  カレンダー未接続 — デフォルト: 平日60分夜練 / 週末120分")
        return
    ...
    for i in range(7):
        ...
        races   = gcal.get("races", [])
        morning = gcal.get("morning_ok", True)
        is_trip = gcal.get("is_trip", False)
        ...
        if races:
            ...
            blocks = max(1, avail // 10)
            bar = "▓" * blocks + "░" * (8-blocks) + f" {avail}分"
            tag = "⚡"
        elif avail <= 60:
            blocks = avail // 10
            bar = "▓" * blocks + "░" * (8-blocks) + f" {avail}分"
            tag = "🌙" if not morning else "🌅"
        elif avail <= 90:
            blocks = min(avail // 10, 8)
            bar = "▓" * blocks + f" {avail}分"
            tag = "✅"
        else:
            bar = "▓▓▓▓▓▓▓▓" + f" {avail}分"
            tag = "💪"
        ...
        note_str = ""
        if notes:
            note_str = "  " + notes[0][:28] if notes else ""

        print(f"  {day_str}({dow}) {today_mark} {tag} {bar}{note_str}")
        if races:
            for race in races:
                gt = ""
                if race.get("past_time_str"):
                    goal_s = int(race.get("past_time_s",0)*0.97)
                    gt = f"  目標:{_fmt_time(goal_s)}"
                elif race.get("rival_time_str"):
                    gt = f"  目標:{race['rival_time_str']}"
                print(f"            → {race['name']}{gt}")

        if avail > 0 and not races:
            total_avail += avail
            session_days += 1

    print()
    h, m = divmod(total_avail, 60)
    print(f"  週合計練習可能: {h}h{m:02d}m  練習予定: {session_days}日")

    # 注目ポイント
    trip_days = [gcal_days[d] for d in gcal_days
                 if gcal_days[d].get("is_trip") and
                 date.fromisoformat(d) >= start_date and
                 date.fromisoformat(d) < start_date+timedelta(days=7)]
    if trip_days:
        print(f"  ✈️ 移動日あり → 前後日も負荷軽減済み")
...
def main():
    ...
    if args.today:
        ...
        print_plan(plan, ri, cond, athlete, gt, cfg, today_mode=True, str_prog=str_prog)
        print("="*64)
        return
    ...
    while True:
        ...
        print_plan(plan, ri, cond, athlete, gt, cfg, str_prog=str_prog)
        print(f"{'═'*64}")
...
smart_plan_v6.py と同じフォルダに coach_chat.html を配置してください。</p>
...
def _fetch_gcal_events_auto():
    ...
    定期的に「python smart_plan_v6.py --sync-gcal」で更新してください。
    """
    import os
    ...
    # ── フォールバック（最終同期: 2026-03-09 ← Googleカレンダーから自動取得）──
    # ※ このリストは claude.ai の Google Calendar MCP から取得した実データです。
    #   内容が変わった場合は「python smart_plan_v6.py --sync-gcal」で更新してください。
    return [
        # 在宅勤務（練習時間+30分）
        {"summary":"在宅","start":{"date":"2026-03-09"},"end":{"date":"2026-03-10"}},
        {"summary":"在宅","start":{"date":"2026-03-10"},"end":{"date":"2026-03-11"}},
